...
const Buy = () => {
  ...
  const fetchProperties = async (newPage = 1, newFilters = filters) => {
    setIsLoading(true)
    try {
      const res = await api.get("/properties/rent", {
        params: {
          page: newPage,
          ...newFilters,
        },
      ...
      <div className="bg-gray-50 min-h-screen">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="pt-10 pb-6">
            <h1 className="text-3xl font-extrabold tracking-tight text-gray-900">Properties for Rent</h1>
            <p className="mt-2 text-sm text-gray-500">Find your dream property in the most desirable locations</p>
          </div>
            ...
            <div className="fixed inset-0 flex z-40 lg:hidden">
              ...
              <div className="ml-auto relative max-w-xs w-full h-full bg-white shadow-xl py-4 pb-12 flex flex-col overflow-y-auto">
                ...
                <div className="mt-4 border-t border-gray-200">
                  <Disclosure title="Property Type" icon={<Home className="h-4 w-4 mr-2 text-gray-500" />}>
                    <div className="space-y-4">
                      {propertyTypes.map((option) => (
                        <div key={option.id} className="flex items-center">
                          ...
                          />
                          <label htmlFor={`filter-mobile-${option.id}`} className="ml-3 min-w-0 flex-1 text-gray-500">
                            {option.name}
                          </label>
                        </div>
                      ))}
